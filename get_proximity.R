...
# feat_key <- "leisure"
# feat_value <- "park"
# feat_title <- "15-Minute City: Park"
# plot_name <- "park"
...
feat_key <- "shop"
feat_value <- c("convenience", "general", "department_store", "supermarket", "mall", "wholesale")
feat_title <- "15-Minute City: Groceries"
plot_name <- "shop"
...
for(year in c(2020, 2050, 2100)){
  ...
  jpeg(paste0("plots/", plot_name, "_map.jpg"), res = 150, width = 1000, height = 700)
  ...
  for(type in c("minority", "income", "education", "age_sex")){
    ...
    res_all[[type]] <- rbind(res_all[[type]],  cbind(res, year = year))
    
    # Plot
    #=====================================================
    
    # Plot (total population)
    jpeg(paste0("plots/", plot_name, "_demo_pop_", type, "_", year, ".jpg"), res = 150, width = 1000, height = 700)
    ...
    # Plot (percentage)
    jpeg(paste0("plots/", plot_name, "_demo_perc_", type, "_", year, ".jpg"), res = 150, width = 1000, height = 500)
...
# Plot (percentage) time series differences


# Make sure 'walkable' is filtered for 'Yes' only to show percentage with access
type <- "minority"

df_filtered <- res_all[[type]] %>%
  filter(walkable == "Yes")

jpeg(paste0("plots/", plot_name, "_proj_", type, ".jpg"), res = 150, width = 1000, height = 600)

ggplot(df_filtered, aes(x = group_title, y = percentage, fill = factor(year))) +
  geom_col(position = "dodge") +
  labs(
    title = feat_title,
    x = "Demographic Group",
    y = "Population with walkable access (%)",
    fill = "Year"
  ) +
